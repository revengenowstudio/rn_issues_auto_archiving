    ...
    response = httpx.post(
        ...
        params={
            "expires_at": (datetime.now() +
                           timedelta(days=token_ttl_days)).isoformat()
        }
    )
    response.raise_for_status()
    new_token: str = response.json()["token"]

    print(Log.rotate_success
          .format(expire_time=response.json()["expires_at"]))
...
def update_repository_variable(
    ...
    '''
    print(Log.update_repo_variable
          .format(variable_name=target_variable_name))
    response = httpx.put(
    ...
    response.raise_for_status()
    print(Log.update_repo_variable_success
          .format(variable_name=target_variable_name))

...
class ArchiveDocument:
    ...
    def file_load(self, path: str):
        print(
            Log.getting_something_from.format(
                another=path, something=Log.archive_document_content
            )
        )
        self.__path = path
    ...
    def __replace_line(self, index: int, line: str) -> None:
        print(
            Log.replaced_line_index.format(
                line_index=index,
            )
        )
    ...
    @staticmethod
    def __find_table_number_in_line(line: str, table_separator: str) -> int:
        result = 0
        start = line.find(table_separator)
        end = line.find(table_separator, start + 1)
        if (temp := line[start + 1 : end]).isdigit():
            print(Log.got_archive_number.format(archive_number=temp))
            result = int(temp)
        else:
            print(
                Log.unexpected_archive_number.format(
                    default_number=result + 1, line=line
                )
            )
        return result

    def __get_last_table_number(self, table_separator: str) -> int:
        return self.__get_table_number_by_line_index(
            self.__get_table_last_line_index(), table_separator
        )
    ...
    def __parse_issue_title(
        ...
        if action_map is None:
            ...
            result = "".join(
                [action_map["add_prefix"], result, action_map["add_suffix"]]
            )
            return result
    ...
    def should_issue_record_exists(
        ...
        for line in self.__lines_set:
            if sub_string in line:
                print(Log.issue_id_found_in_archive_record.format(issue_id=issue_id))
                return True
        print(Log.issue_id_not_found_in_archive_record.format(issue_id=issue_id))
        return False

    def __get_table_number_by_line_index(
        self, line_index: int, table_separator: str
    ) -> int:
        return self.__find_table_number_in_line(
            self.__lines[line_index], table_separator
        )

    def __find_line_index_by_issue_id(
        self, issue_id: int, issue_repository: str
    ) -> int:
    ...
    def archive_issue(
        ...
        if (
            replace_mode
            and (
                line_index := self.__find_line_index_by_issue_id(
            ...
            print(
                Log.replace_old_issue_record.format(
                    issue_id=issue_id, issue_repository=issue_repository
                )
            )
            ...
            table_id = self.__get_last_table_number(table_separator) + 1
        ...
        new_content = archive_template.format(
            ...
            issue_title=self.__parse_issue_title(
                issue_title, issue_type, issue_title_processing_rules
            ),
            rjust_space=((rjust_space_width - len(issue_title)) * rjust_character),
        ...
        if replace_mode and line_index != -1:
            self.__replace_line(line_index, new_content)
        else:
    ...
    def save(self) -> None:
        ...
        if len(self.__new_lines) != 0:
            self.__lines.insert(
                self.__get_table_last_line_index() + 1, *self.__new_lines
            )

...
class TestJsonConfigDataSource:
        ...
        [
            {
                ...
                "issue_type": {"type_keyword": {"#Bug#": "Bug修复"}},
                "archived_document": {
    ...
    def test_load(self, tmp_path: Path, json_data: dict[str, Any]):
        ...
        assert config.archive_necessary_labels == json_data["archive_necessary_labels"]
        assert config.issue_type.type_keyword == json_data["issue_type"]["type_keyword"]
        assert (
            config.archived_document.issue_title_processing_rules
            == json_data["archived_document"]["issue_title_processing_rules"]
        )

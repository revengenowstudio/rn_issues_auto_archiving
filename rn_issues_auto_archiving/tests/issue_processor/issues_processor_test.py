...
class TestIssueProcessor():
    ...
    def test_verify_not_archived_object(
        ...
        with patch(
            ...
            with patch(
                ...
                should_ci_running_in_issue_event_method.return_value = should_ci_running_in_issue_event
                should_ci_running_in_manual_method.return_value = should_ci_running_in_manual
                ...
                if not isinstance(expect_result, bool):
                    with pytest.raises(expect_result):
                        IssueProcessor.verify_not_archived_object(
                            issue_info, config)
                else:
                    assert IssueProcessor.verify_not_archived_object(
                        issue_info, config) == expect_result
    ...
    def test_gather_info_from_issue(self):
        ...
        gather_info = IssueProcessor.gather_info_from_issue(
            issue_info,
            config
        )
    ...
    def test_update_issue_info_with_gather_info(
        self
    ):
    ...
    def test_should_skip_archived_process(self):
        ...
        reges = ["1",    "2"]
        assert IssueProcessor().should_skip_archived_process(
            issue_info, reges
        ) == True

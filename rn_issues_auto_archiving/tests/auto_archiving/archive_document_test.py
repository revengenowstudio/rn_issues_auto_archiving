...
class TestArchiveDocument:
    ...
    @patch("builtins.open")
    def test_file_load(self, mock_open: MagicMock, archive_document: ArchiveDocument):
    ...
    def test_should_issue_record_exists(
        ...
        assert archive_document.should_issue_record_exists("外部Issue", 123)
        assert archive_document.should_issue_record_exists("外部Issue", 1234) is False
    ...
    def test_save(self, archive_document: ArchiveDocument, tmp_path: Path):
        ...
        assert test_file.read_text(encoding="utf-8").split("\n") == test_lines

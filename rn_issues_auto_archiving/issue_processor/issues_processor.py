...
class IssueProcessor:
    ...
    def init_issue_info(
        ...
        if isinstance(platform, GithubClient):
            ...
            issue_info.update(platform_type=platform.name)
        elif isinstance(platform, GitlabClient):
            GitlabIssueDataSource().load(issue_info)
            issue_info.update(platform_type=platform.name)
        else:
            raise UnexpectedPlatform(
                Log.unexpected_platform_type.format(platform_type=type(platform))
            )
        return issue_info
    ...
    def verify_not_archived_object(issue_info: IssueInfo, config: Config) -> bool:
        ...
        if CiEventType.should_ci_running_in_issue_event() and (
        ...
        if issue_info.issue_state == IssueState.update:
            print(Log.issue_state_is_update)
            return True

        running_in_manual = CiEventType.should_ci_running_in_manual()
        not_input_archive_version = issue_info.archive_version == ""
        if (running_in_manual and not_input_archive_version) or not running_in_manual:
            ...
            if not running_in_manual and not_archived_issue:
                ...
                running_in_manual and not_input_archive_version
    ...
    @staticmethod
    def gather_info_from_issue(issue_info: IssueInfo, config: Config) -> GatherInfo:
        ...
        if issue_info.introduced_version == "":
            gather_info.introduced_version = (
    ...
    def update_issue_info_with_gather_info(
        issue_info: IssueInfo, gather_info: GatherInfo
    ) -> None:
        issue_info.update(
    ...
    @staticmethod
    def parse_issue_info_for_archived(issue_info: IssueInfo, config: Config) -> None:
        issue_info.update(
    ...
    def close_issue_if_not_closed(
        ...
        if CiEventType.should_ci_running_in_manual() and (
            issue_info.issue_state == IssueState.open
        ):
            platform.close_issue(issue_info.links.issue_url)
